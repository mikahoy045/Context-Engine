...
if [[ "$BUNDLE_DEPS" == "--bundle-deps" ]]; then
    ...
    if [[ "$(uname -s)" == "Darwin" ]]; then
        ...
        "$PYTHON_BIN" -m pip install --no-warn-conflicts -t "$STAGE_DIR/python_libs" "urllib3<2" requests charset_normalizer "openai>=1.0" watchdog
    else
        "$PYTHON_BIN" -m pip install --no-warn-conflicts -t "$STAGE_DIR/python_libs" requests urllib3 charset_normalizer "openai>=1.0" watchdog
    fi
fi
