...
function activate(context) {
  ...
  try {
    ...
    const graphBackfillCmd = vscode.commands.registerCommand('contextEngineUploader.graphBackfill', async () => {
      ...
      if (!mcpUrl) {
        vscode.window.showErrorMessage('MCP server URL not available');
        return;
      }

      const result = await vscode.window.showWarningMessage(
        'This will trigger graph backfill for the current workspace. This may take a while for large codebases. Continue?',
        'Yes',
        'No'
      );
      ...
      try {
        ...
        if (initResp.ok) {
          ...
          if (initText.includes('event:') || initText.includes('data:')) {
            ...
            if (lastDataLine) {
              initBody = JSON.parse(lastDataLine);
            } else {
        ...
        if (response.ok) {
          ...
          try {
            if (!text || text.trim() === '') {
              ...
              vscode.window.showErrorMessage('Empty response from MCP server');
              return;
            }
            
            if (text.includes('event:') || text.includes('data:')) {
              log('SSE format detected');
              ...
              if (lastDataLine) {
                log(`Parsing data line: ${lastDataLine.substring(0, 100)}...`);
                result = JSON.parse(lastDataLine);
              } else {
                log(`No data line found, parsing full text`);
                result = JSON.parse(text);
              }
            } else {
              log(`No SSE format, parsing as JSON`);
              result = JSON.parse(text);
            }
          } catch (e) {
            log(`Failed to parse response: ${e instanceof Error ? e.message : String(e)}`);
            log(`Response text: ${text.substring(0, 500)}`);
            vscode.window.showErrorMessage(`Failed to parse response: ${e instanceof Error ? e.message : String(e)}`);
            return;
          }

          if (result.error) {
            log(`Graph backfill error: ${JSON.stringify(result.error)}`);
            vscode.window.showErrorMessage(`Graph backfill error: ${result.error.message || 'Unknown error'}`);
          } else {
            log('Graph backfill triggered successfully');
            vscode.window.showInformationMessage('Graph backfill started. Check the output channel for progress.');
          }
        } else {
          log(`Graph backfill failed: ${response.status}`);
          vscode.window.showErrorMessage(`Graph backfill failed: ${response.status}`);
        }
      } catch (error) {
        log(`Graph backfill error: ${error instanceof Error ? error.message : String(error)}`);
        vscode.window.showErrorMessage(`Graph backfill error: ${error instanceof Error ? error.message : String(error)}`);
      }
    });
    context.subscriptions.push(graphBackfillCmd);
